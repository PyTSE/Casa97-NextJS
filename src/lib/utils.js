...
export function createMessage (payload, edit=null) {

  if(edit && payload.itensCarrinho.length > 0){
    const valorTotalCalculado = payload.itensCarrinho.reduce((total, item) => total + item.itemValue, 0);
    const message = `
    *RESERVA ALTERADA!*
    
    Estes são os novos dados da sua reserva:
    ...
    *Data*: ${payload.formattedDate}
    ...
    Itens inclusos:
    ${payload.itensCarrinho.map(item => `- ${item.name} - R$${item.itemValue}`).join('\n')}
  ...
  if(edit === null && payload.itensCarrinho.length > 0){
